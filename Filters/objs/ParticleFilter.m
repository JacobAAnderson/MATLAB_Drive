classdef ParticleFilter
    ...
    methods
        ...
        function obj = ParticleFilter(name, num_Particles)
...
%             obj.derivatives.dh_dn = @(x,u) 1;
        ...
        function obj = Update(obj, speed, dt, heading, measurment)
            ...
            % Sample the map
            depths  = obj.Map.Depth(newParticles);
            
            % Update the weights
            diffs = depths + measurment;
...
%             end
        ...
        % Update Partilce Filter Locations and Weigths from Dead Reckoning with Acomms communication
        function obj = Update_Acoms(obj, speed, dt, heading, measurment, acoms)
            ...
            if any(isnan(diffs)), warning('NaN measurments from bathimetry map'), end
            ...
            distances = x - newParticles;                                   % Project the range measurments onto the new particles
            distances = distances./ vecnorm(distances, 2, 2);
            samplePoints = newParticles + distances * dist;
            
            s = obj.noise.acoms.sigma;
            s = [s*distances(1), 0; 0, s*distances(2)];                     % Include acoms error
           
            cov_ = sqrt( cov_*cov_ + s*s);
            cov_ = (cov_ + cov_')/2;                                        % Make sure the matrix is symmetric positive semi-definite
        ...
        % Update Partilce Filter Locations and Weigths from Acomms communication
        function obj = Acoms(obj, acoms)
